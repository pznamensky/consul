...
Cross-DC control plane communication, also known as WAN federation,
allows for Consul servers in different datacenters to be federated exclusively through mesh gateways.
...
- Consul 1.8.0+
- Mesh federation between Consul datacenters: all datacenters must be connected to all other datacenters
   - Other topologies, such as [hub-and-spoke enabled by Consul Enterprise network areas](https://learn.hashicorp.com/tutorials/consul/federation-network-areas),
     cannot be used
