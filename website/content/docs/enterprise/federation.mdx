...
- [Direct connectivity between all Consul servers](https://www.consul.io/docs/k8s/installation/multi-cluster#traditional-wan-federation)
  - Verify that [`connect.enable_mesh_gateway_wan_federation`](/docs/agent/options#connect_enable_mesh_gateway_wan_federation)
    is set to `false` (default) to ensure cross-datacenter control plane traffic is direct, not
    [funneled through mesh gateways](https://www.consul.io/docs/connect/gateways/mesh-gateway/wan-federation-via-mesh-gateways)
